...
import {
  getLatestTargetSelectorAllowlistHookDeploymentPath,
  getTargetSelectorAllowlistHookArtifactPath,
  readGeneratedArtifact
} from './artifacts.js';
import { getWorkspaceRoot, readTargetSelectorAllowlistHookDeployConfig } from './config.js';
...
async function main() {
  const config = readTargetSelectorAllowlistHookDeployConfig();
  const artifact = readGeneratedArtifact(getTargetSelectorAllowlistHookArtifactPath());
  ...
  if (config.existingHookAddress) {
    ...
    if (existingCode === '0x') {
      throw new Error(
        `No deployed code found at configured ZKSYNC_SEPOLIA_SED_SELECTOR_ALLOWLIST_HOOK_ADDRESS ${config.existingHookAddress}`
      );
    }
  ...
  const record = {
    ...
    contractName: artifact.contractName || 'TargetSelectorAllowlistHook',
