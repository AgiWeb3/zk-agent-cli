...
import {
  getLatestTargetAllowlistHookDeploymentPath,
  getTargetAllowlistHookArtifactPath,
  readGeneratedArtifact
} from './artifacts.js';
import { getWorkspaceRoot, readTargetAllowlistHookDeployConfig } from './config.js';
...
async function main() {
  const config = readTargetAllowlistHookDeployConfig();
  const artifact = readGeneratedArtifact(getTargetAllowlistHookArtifactPath());
  ...
  if (config.existingHookAddress) {
    ...
    if (existingCode === '0x') {
      throw new Error(
        `No deployed code found at configured ZKSYNC_SEPOLIA_SED_TARGET_ALLOWLIST_HOOK_ADDRESS ${config.existingHookAddress}`
      );
    }
  ...
  const record = {
    ...
    contractName: artifact.contractName || 'TargetAllowlistHook',
  ...
  fs.writeFileSync(getLatestTargetAllowlistHookDeploymentPath(), JSON.stringify(record, null, 2), 'utf8');
