...
export function createNextCommand(deps?: Partial<NextCommandDeps>): Command {
  ...
  return new Command('next')
    ...
    .action(async (options: NextCommandOptions) => {
      await runTopLevelNext(options, resolvedDeps);
    });
}

export function createStartCommand(deps?: Partial<NextCommandDeps>): Command {
  const resolvedDeps = resolveNextCommandDeps(deps);
...
async function runTopLevelNext(options: NextCommandOptions, resolvedDeps: NextCommandDeps) {
