...
export function createWalletCommand(deps?: Partial<WalletCommandDeps>): Command {
  ...
  wallet.addHelpText(
    ...
    [
      '',
      'Wallet surface:',
      '  Use this layer when the blocker is specifically wallet approval, signer state, or session recovery.',
      ...
      '    create        -> first local-first bootstrap when no wallet session exists yet',
      '    reapprove     -> the wallet exists but approval/session access must be refreshed',
      '    signer attach -> approval still exists but local write readiness is missing',
      '    status / next -> the blocker is clearly wallet-scoped but the exact repair step is still unclear',
      '    suite         -> wallet readiness is already clear and the question is broader than wallet recovery',
      '',
      '  First local-first bootstrap:',
      '    zk-agent wallet create --await-local',
      '    zk-agent next',
      '',
      '  Repair an existing wallet session:',
      '    zk-agent wallet reapprove --name main --await-local',
      '    zk-agent next',
      '',
      '  Repair signer-only local execution state:',
      '    zk-agent wallet signer attach --name main --private-key <hex>',
      '    zk-agent next',
      '',
      '  Wallet-scoped diagnosis:',
      '    zk-agent wallet status --name main',
      '    zk-agent wallet next --name main',
      '',
      '  Switch to the packaged catalog after wallet readiness:',
      '    zk-agent suite',
      '',
      '  Hosted remote approval only when the browser is remote:',
